...
def generate_popup(text, loc):  # Don't use single apostophe
    html = "<body><script type=\"text/javascript\">alert('" + text + \
        "'); window.location.href = '" + loc + "';</script></body>"
    return html
...
def IsPhoneNumberDuplicate(PhoneNumber):
    ...
    SQLcursor.execute(
        "SELECT \"phoneNumber\" from users WHERE \"phoneNumber\" in %(g)s ", params)
...
def GetPhoneNumber(userID):
    ...
    SQLcursor.execute(
        "SELECT \"phoneNumber\" from users WHERE \"userID\" in %(p)s ", params)
...
def GetUserName(userID):
    params = {'p': tuple([userID])}
    SQLcursor.execute(
        "SELECT \"userName\" from users WHERE \"userID\" in %(p)s ", params)
    for row in SQLcursor:
        return row[0]
...
def GetHealthcareStatus(userID):
    ...
    SQLcursor.execute(
        "SELECT \"HealthcareOveride\" from users WHERE \"userID\" in %(h)s ", params)
    for row in SQLcursor:
        if row[0] == False:
...
def CheckVerificationCode(CodeToCheck):  # returns true false and user id
    ...
    params = {'l': l}
    SQLcursor.execute(
        'SELECT \"UserID\" FROM \"VerificationCode\" WHERE \"VerificationCode\" in %(l)s', params)
    if SQLcursor.rowcount > 0:
        truefalse = True
...
def RemoveVerificationCode(UserID):
    params = {'g': tuple([UserID])}
    SQLcursor.execute(
        "DELETE FROM public.\"VerificationCode\" WHERE \"UserID\" in %(g)s", params)
    conn.commit()
...
def ChangeUserActivation(UserID):
    ...
    params = {'g': g}
    SQLcursor.execute(
        "UPDATE public.users SET \"NumberVerified\"= True WHERE \"userID\" in %(g)s", params)
    conn.commit()
...
def QRpersonalFunc():
    ...
    SQLcursor.execute(
        'SELECT * FROM \"Appointments\" WHERE \"userID\" in %(y)s', params)
    for row in SQLcursor.fetchall():
        ApptTime = str(row[1])
        break
    # print(ApptTime)
...
def TableData():
    ...
    SQLcursor.execute(
        'SELECT * FROM \"Appointments\" WHERE \"businessID\" in %(y)s', params)
    ...
    for row in AppointmentsFetch:
        ...
        SQLcursor.execute(
            "SELECT \"userName\",\"phoneNumber\",\"HealthcareOveride\" from users WHERE \"userID\" in %(g)s ", params)
        for item in SQLcursor.fetchall():
            ...
            if count == 0:
                count = count+1
                PluralAppts.append(Appointment(
                    username=item[0], phonenumber=item[1], healthcare=item[2], AppTime=AppointmentTime))
            else:
                PluralAppts.append(Appointment(
                    username=item[0], phonenumber=item[1], healthcare=item[2], AppTime=AppointmentTime))
...
def businessSignup():
    ...
    passwordhash = bcrypt.hashpw(pass1.encode(
    ...
    InsertData = (UniqueID, LengthOfSession, AmountOfSlots,
                  False, passwordhash, Latitude, Longitude)
...
def businessLogin():
   ...
   # passwordhash = bcrypt.hashpw(password.encode('utf-8'),bcrypt.gensalt(12)).decode('utf-8')
    ...
    resp = make_response(generate_popup(
        ...
        if item[1] == UniqueID:
           # print(passwordhash)
            if bcrypt.checkpw(password.encode("utf-8"), item[0].encode("utf-8")):
                resp = make_response(generate_popup(
                    "You have logged in successfully. You will now be taken to the dashboard.", "/business/dashboard"))
...
def signupAPI():
    ...
    if 'signupCheck' in request.cookies:
        # previous signup attempt - cookie should be overwritten
        signupCookie = request.cookies["signupCheck"]
    ...
    # decode done in same manner then decode the string
    passwordhash = bcrypt.hashpw(pass1.encode(
        'utf-8'), bcrypt.gensalt(12)).decode('utf-8')
...
def AddNewShopRequest():
    ...
    if "shopID" in request.form:
        ...
        SQLcursor.execute(
            "INSERT INTO \"ShopRequests\"(\"GoogleIdentity\") VALUES (%(t)s)", parmas)
        conn.commit()
...
def ReturnMatches():
    ...
    while len(Offers) < 6:
        ...
        params = {"lat": tuple([currentLocation["lat"]]), "long": tuple(
            [currentLocation["lng"]]), "radius": tuple([radius]), "userID": tuple([userID])}  # parameters for quert
        ...
        SQLsmt = ("SELECT * FROM \"Appointments\" WHERE \"businessID\" IN" +
                  str(tuple(LocalBusinessess))+" AND \"userID\" IS NULL")
...
def BookSlot():
    ...
    SQLcursor.execute(
        "SELECT \"phoneNumber\" from users WHERE \"userID\" in %(u)s ", params)
    returneddata = SQLcursor.fetchall()
    ...
    SQLcursor.execute(
        "DELETE FROM \"Appointments\" WHERE \"userID\" in %(u)s  AND \"appointmentID\" ! in %(a)s ", params)
    return "slot booked"
...
def loginAPI():
    if "number" in request.form and "password" in request.form:
        # request so passes in to the form as '+4475...'
        number = request.form["number"]
        password = request.form["password"]
...
def midnightRun():
    ...
    while True:
        now = datetime.now()
        tfmidnight = (now - now.replace(hour=0, minute=0,
        ...
        # table cleared
        SQLcursor.execute("DELETE FROM \"Appointments\ RETURNING *")
        SQLcursor.execute(
            "SELECT \"GoogleIdentity\",\"LengthOfSlot\",\"SlotsPerHour\" FROM \"Establishments\"")
        ...
        for row in EstabLi:
            ...
            if(response.get("result")):
                ...
                OpeningTime = datetime.strptime(
                    OpeningTime, '%I:%M %p')  # .strftime("%H:%M")
                ClosingTime = datetime.strptime(
                    ClosingTime, '%I:%M %p')  # .strftime("%H:%M")

                # -timedelta(hours =0,minutes=int(row[1]))
                CurrentTimeCount = OpeningTime
                LastSlot = ClosingTime-timedelta(hours=0, minutes=int(row[1]))
                while CurrentTimeCount != LastSlot:
                    for i in range(0, int(row[2])):
                        SQLstmt = ("INSERT INTO \"Appointments\"(\"startTime\",\"businessID\") VALUES('" +
                                   CurrentTimeCount.strftime("%H:%M")+"','"+row[0]+"')")
                        SQLcursor.execute(SQLstmt)
                        CurrentTimeCount = CurrentTimeCount + \
                            timedelta(hours=0, minutes=int(row[1]))
